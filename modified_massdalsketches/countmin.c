  ...
  if (cm && prng)
    {
      ...
      cm->width=width;
      cm->count=0;
      cm->counts=(long**)calloc(sizeof(long*),cm->depth);
      cm->counts[0]=(long*)calloc(sizeof(long), cm->depth*cm->width);
      cm->hasha=(char **)calloc(sizeof(char *),cm->depth);
		cm->hashb=(char **)calloc(sizeof(char *),cm->depth);
      for(j=0;j<depth;j++)
      {
        cm ->hasha[j] = allocate_array(sizeofdata);
        cm ->hashb[j] = allocate_array(sizeofdata);
      }
      if (cm->counts && cm->hasha && cm->hashb && cm->counts[0])
	{
	  for (j=0;j<depth;j++)
        ...
        generate_rand_using_prng(cm->hashb[j], prng);
	      ...
	      cm->counts[j]=(long*) cm->counts[0]+(j*cm->width);
        //printf("A -%d\n,B -%d\n,counts -%d\n" ,cm->hasha[j],cm->hashb[j],cm->counts[j]);
	    }
	}
      else cm=NULL;
    }
  ...
  if (cm)
      ...
      if (cm->counts && cm->hasha && cm->hashb && cm->counts[0])
	{
	  for (j=0;j<cm->depth;j++)
	    {
	      strcpy(cm->hasha[j], cmold->hasha[j]);
	      strcpy(cm->hashb[j], cmold->hashb[j]);
	      cm->counts[j]=(long*) cm->counts[0]+(j*cm->width);
	    }
	}
      else cm=NULL;
...
void CM_Update(CM_type * cm, char* item)
{
  ...
  cm->count+=1;
   for (j=0;j<cm->depth;j++){
    //printf("hashed value - %d\n",cm->counts[j][hash31(cm->hasha[j],cm->hashb[j],item)% cm->width]);

    cm->counts[j][(long long)hashval(item,cm->hasha[j],cm->hashb[j],cm->width)]+=1;
    //printf("new hashed value - %d\n",cm->counts[j][hash31(cm->hasha[j],cm->hashb[j],item)% cm->width]);
  }
  // this can be done more efficiently if the width is a power of two
}

long CM_PointEst(CM_type * cm, char* query)
{
  // return an estimate of the count of an item by taking the minimum
  long j, ans;

  if (!cm) return 0;
  ans=cm->counts[0][(long long)hashval(query,cm->hasha[0],cm->hashb[0],cm->width)];
  for (j=1;j<cm->depth;j++)
    //printf("Count -%li\n",cm->counts[j][(long long)hashval(query,cm->hasha[j],cm->hashb[j],cm->width)]);
    ans=min(ans,cm->counts[j][(long long)hashval(query,cm->hasha[j],cm->hashb[j],cm->width)]);
  // this can be done more efficiently if the width is a power of two
  return (ans);
}

long CM_PointMed(CM_type * cm, char* query)
{
  ...
  if (!cm) return 0;
  ans=(long *) calloc(1+cm->depth,sizeof(long));
  for (j=0;j<cm->depth;j++)
    ans[j+1]=cm->counts[j][(long)hashval(query,cm->hasha[j],cm->hashb[j],cm->width)];
...
{ // test whether two sketches are comparable (have same parameters)
  ...
  for (i=0;i<cm1->depth;i++)
    {
      if (!strcmp(cm1->hasha[i],cm2->hasha[i])) return 0;
      if (!strcmp(cm1->hashb[i],cm2->hashb[i])) return 0;
    }
  return 1;
}
...
/*
int CM_Residue(CM_type * cm, char **Q)
{
