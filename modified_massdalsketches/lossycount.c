...
{  // merge up two lists of counters. returns the size of the lists. 
  ...
  while (i<l && j<r)
	...
	{ // sum the counts of identical items
	  ...
	  while (strcmp(left[i].item, right[j].item)==0)
	    {
	...
	{ // else take the left item, creating counts appropriately
	  newcount[m].item=left[i].item;
	  newcount[m].count=0;
	  while (strcmp(left[i].item, newcount[m].item)==0)
	    {
	      newcount[m].count+=left[i].count;
	      i++;
	    }
	...
	while(i<l)
	    {
	      newcount[m].item=left[i].item;
	      newcount[m].count=-1;
	      while (strcmp(newcount[m].item, left[i].item)==0 && (i<l))
		{
		  newcount[m].count+=left[i].count;
		  i++;
		}
...
void LC_Update(LC_type * lc, char* val)
{
  Counter *tmp;
