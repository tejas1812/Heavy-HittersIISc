...
int Freq_Size(freq_type * freq)
{
  ...
  while (g!=NULL) 
	...
	do 
	  {
	    size += strlen(i->item)*sizeof(char);
	    numitems +=1;
	    i=i->nexting;
	  }
	while (i!=first);
  ...
  size += 2*sizeof(int) + sizeof(char)*(strlen(freq->a) + strlen(freq->b));
  printf("Frequent size: %d\n", size);
  printf("# of items stored in Frequent: %d\n", numitems);
  return size;
}
